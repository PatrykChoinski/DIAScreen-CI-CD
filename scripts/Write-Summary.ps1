<#
.SYNOPSIS
    Reads whichever of reports/junit-{build,test}.xml exist and writes one
    Markdown report to $env:GITHUB_STEP_SUMMARY (the run's Job Summary) -
    or to the console when run locally. Safe to call when earlier stages
    never ran (their report just doesn't exist).
#>
param(
...
$stages = @(
    @{ Name = "Build (otwarcie PilaJednosuportowaINNER.dpa + Compile)"; File = "junit-build.xml" }
    @{ Name = "Test (On-line Simulation przez 30 s)"; File = "junit-test.xml" }
)

$lines = New-Object System.Collections.Generic.List[string]
$lines.Add("## DIAScreen CI - raport")
$lines.Add("")
$anyFailure = $false
$anyRan = $false
...
$lines.Add("_Zrzuty ekranu kazdego etapu, pelny Output kompilatora i log instalacji: artefakt ``diascreen-ci-reports``._")
